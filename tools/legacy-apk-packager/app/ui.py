...
Android 端保持离线；在线歌词只在 PC 侧生成本地 .lrc。
"""
...
class MainWindow(QWidget):
    def __init__(self) -> None:
        ...
        prepare_btn = QPushButton("生成 songs.json 并复制资源")
    ...
    def _fetch_lyrics(self) -> None:
        ...
        tracks = generate_tracks_from_dirs(music_dir, lyrics_dir=lyric_dir)
        ...
        for track in missing:
            stem = Path(track["audio"]).name
            stem = Path(stem).stem
            dest = lyric_dir / f"{stem}.lrc"
            try:
                saved = fetch_and_save(track["title"], track.get("artist", ""), dest)
                if saved:
                    fetched += 1
            except Exception as exc:  # noqa: BLE001
                self._append(f"[WARN] 补歌词失败 {stem}: {exc}")
    ...
    def _prepare_assets(self) -> None:
        ...
        self._append(f"已生成 songs.json，曲目数 {len(tracks)}，并复制资源到 LMusic。")

    def _build(self, build_type: str) -> None:
        try:
            apk_path: Optional[Path] = build_apk(build_type)
        except Exception as exc:  # noqa: BLE001
            self._error(f"打包失败：{exc}")
            return
